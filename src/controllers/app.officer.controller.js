...
exports.getOfficerGroups = async (req, res) =>{
    ...
    var api_endpoint = `${access.apiurl || apiurl}app/getGroups/${access.code}/${access.key}/?page=${page}&limit=${limit}`;
    console.log(api_endpoint);
    var request = unirest('GET', api_endpoint)
        .end(async (resp) => {
            if (resp.error) {
...
exports.getAccountTransaction =  async (req, res) =>{
    ...
    const access =  getkey(req.user.merchant);
    if(!access) res.status(500).send({success: false, message: `No merchant was found with code ${val.merchant}`});
    var api_endpoint = `${access.apiurl || apiurl}accounts/transactions/${id}`;
    var request = unirest('GET', api_endpoint)
        .end(async (resp) => {
            if (resp.error) {
                res.status(500).send({
...
exports.getTransactions =  async (req, res) =>{
    const access =  getkey(req.user.merchant);
    if(!access) res.status(500).send({success: false, message: `No merchant was found with code ${val.merchant}`});
    var api_endpoint = `${access.apiurl || apiurl}app/getTransactions/${access.code}/${access.key}`;
    console.log(api_endpoint);
    var request = unirest('GET', api_endpoint)
        .end(async (resp) => {
            if (resp.error) {
...
exports.createTransaction = async (req, res)=>{
    ...
    if(req.user.agentid) { value.agentid = req.user.agentid; }
    if(!access) res.status(500).send({success: false, message: `No merchant was found with code ${val.merchant}`});
    var api_endpoint = `${access.apiurl || apiurl}ussd/agent/deposit/${access.code}/${access.key}`;
    var request = unirest('POST', api_endpoint)
        .headers({
            'Content-Type': 'application/json'
        })
